...
export function IntroCard({ languages, myTime }) {
 ...
 <SectionCard as="aside" className={`${CARD_CONTAINER} bg-[rgba(248,242,235,0.5)]  backdrop-blur-md min-h-[220px] lg:min-h-[240px] h-full`}>      <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="mt-4 rounded-2xl border-2 border-[#1d1d1b] bg-gradient-to-br from-[#fffdf8] to-[#f4ecdd] p-3.5"><div className="mb-2 flex items-center justify-between"><p className={SECTION_TITLE + ' text-[11px] font-semibold tracking-[0.12em] text-[#2f2a22]'}>Languages</p><p className="text-[10px] text-[#5f5648]">Hover chips</p></div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className={`${BORDERED_PANEL} px-4 py-3 text-center`}><p className="text-sm uppercase tracking-wide">My Time</p><p className="mt-1 text-lg font-medium">{myTime.toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit' })}</p></div><div className={`${BORDERED_PANEL} px-4 py-3 text-center`}><p className="text-sm uppercase tracking-wide">Your Time</p><p className="mt-1 text-lg font-medium">{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div></div>
    </SectionCard>
  );
}
